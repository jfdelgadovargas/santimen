...
@Component({
  ...
  styleUrls: ['../../../theme/utilities.scss', './pizzas.page.scss'],
})
export class PizzasPage implements OnInit {
  ...
  async detail(item){
    this.menuService.getMenuDetail('pizzas', item.id).subscribe(opciones => {
    ...
    const modal = await this.modalCtrl.create({
      ...
      componentProps: {
        ...
        precio: item.precio,
        imagen: item.imagen,
        opciones: this.menuService.getMenuDetail('pizzas', item.id)
      }
    });
